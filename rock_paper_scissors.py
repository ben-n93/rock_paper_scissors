...
def rock_paper_scissors_callout():
    """Function that prints the 'players' calling out the name of the
    'shapes'"""
...
# Main gamelay loop.
while True:
    # Using choice function from random module to pick a shape for computer
    # opponent.
    ...
    if new_game == 'y' or new_game == 'yes':
        ...
        # If user chooses/enters shape not in shapes list, prints error and
        # restarts while loop.
        if user_choice_input not in shapes:
            print("That is not a valid entry!")
            continue
        # Checks to see if user's shape choice and computer's shape is the
        # same, in which case it is a tie.
    ...
    # If user enters q or quit, game/application ends.
        ...
        break
    # If user enters anything other than y/yes, n/no or q/quit, prints error
    # message and restarts while loop.
    else:
